...
let content = std::fs::read(path)
                .map_err(|e| StateError::Io(std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to read file for hashing: {}", e))))?;
...
StateError::Io(std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to read template for hashing: {}", e)))
            })?;
