//! Proxmox LXC lifecycle module (API-driven, local execution).
...
/// Allowed parameters for LXC creation (POST /nodes/{node}/lxc)
...
fn request_json_optional(
    ...
    if !raw.status.is_success() {
        if is_missing_lxc_response(raw.status, &raw.body) {
            return Ok(None);
        }
        return Err(ModuleError::ExecutionFailed(format!(
            "Proxmox API error {} {}: {}",
...
fn is_missing_lxc_response(status: reqwest::StatusCode, body: &str) -> bool {
...
fn api_url(config: &ProxmoxLxcConfig, path: &str) -> String {
...
fn fetch_status_optional(
    ...
    Ok(Some(LxcStatus {
        power_state: LxcPowerState::from_str(status),
        raw: data,
    }))
}
...
fn build_create_params(
    ...
    if !body.contains_key("hostname") {
        if params.get_bool_or("auto_name", false) {
            body.insert(
                "hostname".to_string(),
                format!("rustible-ct-{}", config.vmid),
            );
        } else {
            return Err(ModuleError::MissingParameter(
                "hostname (required for create)".to_string(),
            ));
        }
    }
...
fn build_clone_params(
    ...
    if !body.contains_key("hostname") && params.get_bool_or("auto_name", false) {
        body.insert(
            "hostname".to_string(),
            format!("rustible-ct-{}", config.vmid),
        );
...
fn create_lxc(
    ...
    let url = api_url(config, &format!("nodes/{}/lxc", config.node));
...
fn clone_lxc(
    ...
    let url = api_url(
        config,
        &format!("nodes/{}/lxc/{}/clone", config.node, source_vmid),
    );
...
/// Proxmox LXC lifecycle module.
pub struct ProxmoxLxcModule;

impl Module for ProxmoxLxcModule {
    fn name(&self) -> &'static str {
        "proxmox_lxc"
    }

    fn description(&self) -> &'static str {
        "Manage Proxmox VE LXC lifecycle via the Proxmox API"
    }
    ...
    fn optional_params(&self) -> HashMap<&'static str, serde_json::Value> {
        ...
        params.insert("hostname", json!("ct-name"));
    ...
    fn validate_params(&self, params: &ModuleParams) -> ModuleResult<()> {
        let _ = ProxmoxLxcConfig::from_params(params)?;
        Ok(())
    }
    ...
    fn execute(
        ...
        let config = ProxmoxLxcConfig::from_params(params)?;
        ...
        let mut output = match config.state {
            DesiredState::Status => {
                if status_opt.is_some() {
                    ModuleOutput::ok("Container status retrieved")
                } else {
                    ModuleOutput::ok("Container not found")
                }
            }
            DesiredState::Present => {
                if status_opt.is_some() {
                    ModuleOutput::ok("Container already exists")
                } else if context.check_mode {
                    let _ = build_create_params(&config, params)?;
                    ModuleOutput::changed("Container would be created (check mode)")
                } else {
                    let response = create_lxc(&client, &config, params)?;
                    let output = ModuleOutput::changed("Container create requested")
            ...
            DesiredState::Absent => {
                if status_opt.is_some() {
                    if context.check_mode {
                        ModuleOutput::changed("Container would be deleted (check mode)")
                    } else {
                        let response = delete_lxc(&client, &config, params)?;
                        let output = ModuleOutput::changed("Container delete requested")
                            .with_data("action", json!("delete"))
                            .with_data("action_response", response.clone());
                        handle_task_wait(&client, &config, &response, output)?
                    }
                } else {
                    ModuleOutput::ok("Container already absent")
                }
            }
            DesiredState::Cloned => {
                if status_opt.is_some() {
                    ...
                    ModuleOutput::changed("Container would be cloned (check mode)")
                } else {
                    let source_vmid = parse_vmid_from_param(params, "clone_from")?;
                    let response = clone_lxc(&client, &config, params, source_vmid)?;
                    let output = ModuleOutput::changed("Container clone requested")
            ...
            DesiredState::Started => {
                let status = status_opt.as_ref().ok_or_else(|| {
                    ModuleError::ExecutionFailed("Container not found".to_string())
                })?;
                if status.power_state.is_running() {
                    ModuleOutput::ok("Container already running")
                } else if context.check_mode {
                    ModuleOutput::changed("Container would be started (check mode)")
                } else {
                    let response = perform_action(&client, &config, "start")?;
                    let output = ModuleOutput::changed("Container start requested")
            ...
            DesiredState::Stopped => {
                ...
                if status.power_state.is_stopped() {
                    ModuleOutput::ok("Container already stopped")
                } else if context.check_mode {
                    ModuleOutput::changed("Container would be stopped (check mode)")
                } else {
                    let response = perform_action(&client, &config, config.stop_method.endpoint())?;
                    let output = ModuleOutput::changed("Container stop requested")
            ...
            DesiredState::Restarted => {
                ...
                if context.check_mode {
                    ModuleOutput::changed("Container would be restarted (check mode)")
                } else if status.power_state.is_running() {
                    let response = perform_action(&client, &config, "reboot")?;
                    let output = ModuleOutput::changed("Container reboot requested")
...
mod tests {
    ...
    fn test_parse_string_map() {
        ...
        assert_eq!(parsed.get("hostname"), Some(&"test-ct".to_string()));
    ...
    fn test_validate_params_valid() {
        let mut params = HashMap::new();
        params.insert("hostname".to_string(), "test-ct".to_string());
    ...
    fn test_strict_params_catches_typo_in_create() {
        ...
        let mut params: ModuleParams = HashMap::new();
        params.insert("hostname".to_string(), json!("test-ct"));
